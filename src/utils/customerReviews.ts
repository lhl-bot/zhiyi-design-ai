...
export function reviewSummaryForPrompt(customer: CustomerProfile): string {
  ...
  return reviews.slice(0, 3).map((review) => {
    ...
    return `${review.source}${channelTag}（${reviewStatusForPrompt(review)}，${review.productScope}，可信度${review.confidence}）：${[sample, praised, pain, `辅助出图：${review.designAction}`, caution].filter(Boolean).join("；")}`
  }).join("。")
}
